...
for (i in 1:number_of_patients_text) {
  ...
  discharge_icd9_data[i,1] = gsub(" +"," ",discharge_icd9_data[i,1])
  discharge_icd9_data[i,2] = paste(DIAGNOSES_ICD_csv[inexs_of_icd9_for_patient_i,5],  collapse = " ")
}
colnames(discharge_icd9_data)<-c("text","label")
write.csv(
  discharge_icd9_data,
  "/home/sepideh/Documents/data/all_discharge_with_preprocessing_all_labels.csv",
  row.names = FALSE
)
